...
$('source-copy').insertAdjacentHTML('afterbegin',`<p><strong>Classic comparison:</strong> Talent trees and values from <a href="https://wago.tools/db2/Talent?build=${DATA.classicBuild}" target="_blank" rel="noopener">Classic Era ${DATA.classicBuild}</a>. The class spell baseline is defined using <a href="https://wago.tools/db2/SkillLineAbility?build=${DATA.referenceBuild}" target="_blank" rel="noopener">Classic ${DATA.referenceBuild} before SoD</a>. Seasonal and other extra data already present in the Era client can be displayed separately.</p><p>Talents are matched by spell ID first, otherwise by identical name. Spell ranks are grouped by name and compared using matching spell IDs. “Unchanged” refers to the compared fields, not all possible server-side rules. Different spell IDs appear as separate variants. Effect base values are normalized to a shared mean-value format to avoid treating format changes as gameplay differences. Effects of triggered subspells are not recursively simulated.</p>`);
